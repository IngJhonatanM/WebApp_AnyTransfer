...
public class ResetPasswordModel(UserManager<AppUser> userManager) : PageModel
{
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        if (await _userManager.FindByEmailAsync(Input.Email) is not AppUser appUser)
        {
            // Don't reveal that the appUser does not exist
            return RedirectToPage("./ResetPasswordConfirmation");
        }

        IdentityResult result = await _userManager.ResetPasswordAsync(appUser, Input.Code, Input.Password);
